...
export function parseDate(dateString: any): Date {
  ...
  for (const format of formats) {
    ...
    if (match) {
      let year, month, day, hour = 0, minute = 0, second = 0, period;
      
      if (format === formats[0]) {
        // DD/MM/YYYY format with time, seconds, and AM/PM
        [, day, month, year, hour, minute, second, period] = match;
        const isPM = period?.toLowerCase() === 'pm';
        hour = parseInt(hour) + (isPM && hour !== '12' ? 12 : 0);
        if (isPM && hour === 12) hour = 12;
        if (!isPM && hour === 12) hour = 0;
      } else if (format === formats[1]) {
        // DD/MM/YYYY format with time (no seconds) and AM/PM
        [, day, month, year, hour, minute, period] = match;
        const isPM = period?.toLowerCase() === 'pm';
        hour = parseInt(hour) + (isPM && hour !== '12' ? 12 : 0);
        if (isPM && hour === 12) hour = 12;
        if (!isPM && hour === 12) hour = 0;
...
export async function processDashboardData(timeRange: string = 'all'): Promise<DashboardData> {
  try {
    ...
    try {
      ...
      console.error(`❌ Error calculating booking stats:`, error.message);
    ...
    try {
      ...
      console.error(`❌ Error generating time series:`, error.message);
    ...
    try {
      ...
      console.error(`❌ Error generating category data:`, error.message);
